...
SWEP.ViewModel             = "models/nox/thermaldet/c_thermaldet.mdl"
SWEP.WorldModel            = "models/nox/thermaldet.mdl"
SWEP.UseHands 			   = true
...
function SWEP:Think()
   ...
   if self:GetPin() and self.PrimaryThrow then  

				if self.played == false then
				local throwsound = "weapons/thermaldet/Grenade_Charge.wav"
   ...
   if self:GetPin() and not self.PrimaryThrow then
...
function SWEP:Throw()
   ...
   elseif SERVER then
      ...
      local vel = math.min(250, (90 - tang.p) * 6)
...
function SWEP:GetGrenadeName()
   return "nox_thermaldetonator_proj"
end
...
function SWEP:OnRemove()
   ...
   if CLIENT and IsValid(self:GetOwner()) and self:GetOwner() == LocalPlayer() and self:GetOwner():Alive() then
      RunConsoleCommand("use", "weapon_holster")
   end
end
